...
- **The Grandmaster is the Expert.** The manual lives on their dashboard as the
  **bomb console**. On an ordinary board it is the manual and *nothing else* —
  ...
  page has nothing to keep in step. On a **dark-fuse** board (the bonus-only tiers
...
- **And on the bonus tiers, the clock as well.** From `DARK_FUSE_FROM_LEVEL`
  (11) the payload sets `hidden_deadline`, the timer cell reads `--`, and the renderer
  ...
  Grandmaster's console is the only place it can be read. The dark fuse lines up
  exactly with banks, so a dark-fuse board and a multi-bank board are the
